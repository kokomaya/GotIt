...
- [x] **1.1.3** 实现VAD（Voice Activity Detection）：
  - 基于RMS音量阈值的简单VAD
  - 静音超过1.5秒自动停止录音
- [x] **1.1.4** 音频采集验证通过（sounddevice设备列表+录音功能正常）
...
- [x] **1.2.4** STT功能验证通过（模型加载+转写正常）
- [x] **1.2.5** 性能基准测试：base模型CPU推理 ~2s（静音片段，实际语音会更快，Phase 5优化）
...
- [x] **1.6.4** 实现CLI入口 `gotit/main.py`：
  ...
  - `--debug` 调试日志
- [x] **1.6.5** 端到端集成测试（--text模式验证通过）
- [x] **1.6.6** 创建 git commit：`feat: MVP command-line version`

### Phase 1 验收标准

- [x] `uv run gotit --text "打开记事本"` → 记事本成功启动
- [x] `uv run gotit --text "搜索py文件"` → 返回20条结果
- [x] STT模型加载+转写验证通过
- [x] 所有单元测试通过（**47个测试用例**，远超≥15目标）
- [x] STT基准：base模型CPU ~2s（静音片段偏慢，Phase 5优化目标 <1s）
...
- [ ] **5.4.3** 网络断开 → 自动切换到离线LLM（如已配置Ollama）(不需要支持)
